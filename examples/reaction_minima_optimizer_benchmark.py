#!/usr/bin/env python3
"""QME Reaction Minima Optimizer Benchmark - Minima Optimization Comparison."""
...
def create_minima_structure() -> Atoms:
    """Create a structure for minima optimization using reaction example files."""
    from ase.io import read

    # Use the reaction reactant structure as starting point for minima optimization
    script_dir = Path(__file__).parent
    return read(script_dir / "example_files" / "reaction_001_reactant.xyz")
...
def print_optimizer_summary(results_list: list[dict[str, Any]]) -> None:
    ...
    for results in results_list:
        ...
        status = "✅" if available and converged else "❌"
    ...
    available_results = [r for r in results_list if r.get("available")]
    if available_results:
        optimizer_groups = {}
        for result in available_results:
            opt_name = result.get("optimizer", "unknown")
            optimizer_groups.setdefault(opt_name, []).append(result)
...
def main() -> int:
    """Main function to run the reaction minima optimizer comparison benchmark."""
    interface = QMEExampleInterface(
        name="Reaction Minima Optimizer Benchmark",
        description="Minima Optimization Comparison for reaction_001_reactant.xyz",
