#!/usr/bin/env python3
"""Compare MACE Hessian computation methods against finite differences.
...
def main() -> int:
    ...
    interface = QMEExampleInterface(
        name="MACE Hessian Method Comparison",
        description="MACE Hessian Computation Methods Comparison",
    ...
    # Backend handling: enforce MACE-only
    ...
    backend, available_backends = interface.select_backend(
        ...
        required_backends=["mace"],
        verbose=args.verbose,
    )
    if backend is None or backend != "mace":
        interface.print_error("This demo requires MACE backend. Use --backends mace")
        return 1
    ...
    try:
        ...
        # Set up calculator - use MACE backend
