...
def main():
    ...
    print(
        f"Initial H-H distance: {np.linalg.norm(h2_molecule.coords3d[1] - h2_molecule.coords3d[0]):.3f} Å"
    )

    # Create separated H atoms (product)
    h_separated_coords = np.array([0.0, 0.0, 0.0, 5.0, 0.0, 0.0])  # Large separation
    h_atoms_separated = Geometry(
        atoms=atoms, coords=h_separated_coords, charge=0, mult=3
    )  # Triplet state

    print(
        f"Separated H atoms with distance: {np.linalg.norm(h_atoms_separated.coords3d[1] - h_atoms_separated.coords3d[0]):.3f} Å"
    )

    # Create reaction pathway
    reaction = Reaction(
        reactant=h2_molecule, product=h_atoms_separated, name="H2_dissociation"
    )
