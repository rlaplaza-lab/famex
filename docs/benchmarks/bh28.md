...
- `--ts-optimizer`: Choose the TS optimizer (`SELLA` or `trust-krylov-ts`)
...
- **Transition states**: SELLA optimizer (`pip install sella`) or the built-in `trust-krylov-ts` variant for Hessian-driven searches
...
## Trust-Krylov TS vs Sella Comparison

For a direct, per-reaction comparison between the new `trust-krylov-ts` optimizer and Sella on the BH28 transition states, use the helper script from the project root:

```bash
python scripts/run_bh28_ts_comparison.py
```

The script writes `examples/bh28_benchmark/benchmark_results/trust_krylov_ts_vs_sella_bh28.json` with convergence, force reduction, RMSD, and energy agreement statistics for each reaction.
