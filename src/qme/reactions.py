...
class Reaction:
    ...
    def _distance_matrix_to_coords(
        self, dist_matrix: np.ndarray, reference_coords: np.ndarray
    ) -> np.ndarray:
        ...
        coords_mds = eigenvectors[:, :n_dims] @ np.diag(
            np.sqrt(np.abs(eigenvalues[:n_dims]))
        )
    ...
    def find_transition_state_guess(
        self, npoints: int = 20, method: str = "geodesic"
    ) -> Optional[Geometry]:
