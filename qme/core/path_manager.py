...
class PathManager:
    ...
    def __init__(
        ...
        if len(structures_list) < 2:
            raise ValueError("PathManager requires at least 2 structures")
        ...
        for i in range(len(self.structures) - 1):
            validate_atoms_compatibility(
                self.structures[i], self.structures[i + 1], f"path segment {i}"
            )
    ...
    def find_local_maxima(path: list[Atoms]) -> list[int]:
        ...
        if not maxima_idxs:
            valid = [(i, e) for i, e in enumerate(energies) if not math.isnan(e)]
            if not valid:
                raise RuntimeError("No valid energies found along path to select maxima")
    ...
    def get_rmsd_profile(
        self, path_geometries: list[Geometry] | list[Atoms]
