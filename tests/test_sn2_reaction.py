...
class TestSN2Reaction:
    ...
    def sn2_reactant(self):
        ...
        coords = np.array([
            ...
            -0.37, -0.52, -1.76, # Cl
            # OH- approaching
            3.0, 0.0, 0.0,      # O (far away initially)
            3.97, 0.0, 0.0,     # H
        ])
    ...
    def test_sn2_with_transition_state(self, sn2_reactant, sn2_product, sn2_transition_state):
    ...
    def test_sn2_forces_along_path(self, sn2_reactant, sn2_product):
        ...
        for geom in path_geoms:
            ...
            assert not np.allclose(geom.forces, 0.0), "Forces should not be zero along path"
    ...
    def test_sn2_xyz_export(self, sn2_reactant, sn2_product, sn2_transition_state):
        ...
        xyz_blocks = xyz_traj.split('\n7\n')[1:]  # Split on natoms line, skip first empty
    ...
    def test_sn2_mechanism_validation(self, sn2_reactant, sn2_product):
        ...
        assert np.all(np.diff(c_cl_distances) >= -0.1), "C-Cl distance should generally increase"
        
        # C-O should monotonically decrease
        assert np.all(np.diff(c_o_distances) <= 0.1), "C-O distance should generally decrease"
