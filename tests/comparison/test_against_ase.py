...
def test_qme_vs_ase_hessian_and_frequencies(tmp_path, delta: float) -> None:
    ...
    vib.run()
    vib.read()
    ase_H = vib.H.copy()  # 2D Hessian array in eV/Å^2

    # Compare Hessians (allowing small numerical tolerance)
    assert qme_hess.shape == ase_H.shape == (3 * len(indices), 3 * len(indices))
    # Symmetry checks
    np.testing.assert_allclose(qme_hess, qme_hess.T, rtol=1e-6, atol=1e-6)
    np.testing.assert_allclose(ase_H, ase_H.T, rtol=1e-6, atol=1e-6)
    # Element-wise closeness
    np.testing.assert_allclose(qme_hess, ase_H, rtol=5e-3, atol=5e-3)
