...
from qme import Geometry, HarmonicCalculator, MLPCalculator, Reaction
...
class TestProtonTransferReaction:
    ...
    def test_proton_transfer_setup(
        self, proton_transfer_reactants, proton_transfer_products
    ):
    ...
    def test_proton_transfer_distances(
        self, proton_transfer_reactants, proton_transfer_products
    ):
        """Test key distance changes in proton transfer."""
        reaction = Reaction(
            proton_transfer_reactants, proton_transfer_products, name="PT_distances"
        )
    ...
    def test_proton_transfer_linearity(
        self, proton_transfer_reactants, proton_transfer_products
    ):
        """Test that proton transfer follows approximately linear path."""
        reaction = Reaction(
            proton_transfer_reactants, proton_transfer_products, name="PT_linear"
        )
        ...
        for geom in path_geoms:
            ...
            if np.linalg.norm(nh_vec) > 1e-6 and np.linalg.norm(hcl_vec) > 1e-6:
                cos_angle = np.dot(nh_vec, hcl_vec) / (
                    np.linalg.norm(nh_vec) * np.linalg.norm(hcl_vec)
                )
                ...
                assert (
                    angle > np.pi / 2
                ), f"N-H-Cl angle should be > 90°, got {np.degrees(angle):.1f}°"

    def test_proton_transfer_with_ts(
        self, proton_transfer_reactants, proton_transfer_products, proton_transfer_ts
    ):
        ...
        assert (
            r_nh_dist > ts_nh_dist > p_nh_dist
        ), "TS should be geometrically intermediate"

    def test_proton_transfer_energetics(
        self, proton_transfer_reactants, proton_transfer_products
    ):
    ...
    def test_proton_transfer_forces(
        ...
        for geom in path_geoms:
            ...
            if 1.2 < nh_dist < 2.5:  # In the transfer region
                assert (
                    h_force_magnitude > 0.001
                ), "Transferring proton should experience significant forces"
    ...
    def test_proton_transfer_harmonic_model(self, proton_transfer_products):
        ...
        harmonic_calc = HarmonicCalculator(
            equilibrium_geometry=proton_transfer_products, force_constant=50.0
        )

        # Create slightly displaced geometry
        displaced_geom = proton_transfer_products.copy()
        displaced_geom.coords += np.random.normal(
            0, 0.1, size=displaced_geom.coords.shape
        )
        ...
        assert (
            displaced_geom.energy > 0
        )  # Should be positive (displacement from equilibrium)

        # Forces should point towards equilibrium
        displacement = displaced_geom.coords - proton_transfer_products.coords
        assert (
            np.dot(displacement, displaced_geom.forces) < 0
        ), "Forces should oppose displacement"

    def test_proton_transfer_rmsd_analysis(
        self, proton_transfer_reactants, proton_transfer_products
    ):
        """Test RMSD analysis along proton transfer path."""
        reaction = Reaction(
            proton_transfer_reactants, proton_transfer_products, name="PT_rmsd"
        )
        ...
        assert np.all(
            np.diff(rmsd_from_reactants) >= -0.1
        ), "RMSD from reactants should generally increase"
        assert np.all(
            np.diff(rmsd_from_products) <= 0.1
        ), "RMSD from products should generally decrease"

    def test_proton_transfer_hydrogen_bonding(
        self, proton_transfer_reactants, proton_transfer_products
    ):
        """Test hydrogen bonding interactions during proton transfer."""
        reaction = Reaction(
            proton_transfer_reactants, proton_transfer_products, name="PT_hbond"
        )
        path_geoms = reaction.interpolate(npoints=15)
        ...
        assert (
            min_distance_idx < len(hbond_distances) - 1
        ), "Closest approach should not be at the end"
        assert (
            hbond_distances[min_distance_idx] < 4.0
        ), "Minimum N-Cl distance should suggest interaction"

    def test_proton_transfer_mechanism_validation(
        self, proton_transfer_reactants, proton_transfer_products
    ):
        """Test validation of proton transfer mechanism."""
        reaction = Reaction(
            proton_transfer_reactants, proton_transfer_products, name="PT_mechanism"
        )
        ...
        for i, geom in enumerate(path_geoms):
            ...
            assert (
                3 <= nh_bonds <= 4
            ), f"Should have 3 or 4 N-H bonds, found {nh_bonds} at point {i}"
